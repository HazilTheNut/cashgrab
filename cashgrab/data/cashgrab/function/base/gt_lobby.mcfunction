...
execute as @a[scores={eid_state=0}] run function cashgrab:util/pe_eid_acquire

# Detect players returning to the server after leaving
tag @a[scores={__iev_logout=1..}] add t_return_to_lobby
execute as @a[scores={__iev_logout=1..}] run function cashgrab:base/pe_cleanup
...
execute as @a[scores={__iev_death=1..}] run function cashgrab:base/pe_cleanup
execute as @a[scores={__iev_death=1..}] run function cashgrab:base/pe_create_coinshower
...
execute if score NUM_PLAYERCOUNT_CHANGED num matches 1 as @e[scores={eid_state=1..}] run function cashgrab:base/pe_cleanup_if_ownerless
