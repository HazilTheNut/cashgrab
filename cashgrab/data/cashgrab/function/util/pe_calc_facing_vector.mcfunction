# util/pe_calc_facing_vector.mcfunction
#
# Context:
#	as: an entity
#	at: the entity
...
function coinwars:util/pe_record_position

# Record position immediately ahead of missile (scaled by magnitude arg)
$summon minecraft:marker ^ ^ ^$(magnitude) {Tags:["t_velcalc"]}
$execute positioned ^ ^ ^$(magnitude) run tp @e[tag=t_velcalc,limit=1] ~ ~ ~ facing ^ ^ ^1
execute as @e[tag=t_velcalc,limit=1] at @s run function coinwars:util/pe_record_position

# Set this.facing_vector = t_velcalc.pos
scoreboard players operation @s facing_vector_x_mm = @e[tag=t_velcalc,limit=1] pos_x_mm
scoreboard players operation @s facing_vector_y_mm = @e[tag=t_velcalc,limit=1] pos_y_mm
scoreboard players operation @s facing_vector_z_mm = @e[tag=t_velcalc,limit=1] pos_z_mm
...
execute store result score @s facing_vector_yaw_mdeg run data get entity @e[tag=t_velcalc,limit=1] Rotation[0] 1000
execute store result score @s facing_vector_pitch_mdeg run data get entity @e[tag=t_velcalc,limit=1] Rotation[1] 1000

kill @e[tag=t_velcalc,limit=1]