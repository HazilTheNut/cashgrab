...
execute if entity @a[tag=t_pm_owner,scores={ability_charges=1..,activity_state=20..29}] run function cashgrab:classes/pmt_class_inv_ability_icon with entity @s data.class_info
execute if entity @a[tag=t_pm_owner,scores={ability_charges=1..,activity_state=30..39}] run function cashgrab:tutorial/trainee/pmt_trainee_inv_ability_icon with storage cashgrab:abi_args
