...
execute if entity @e[tag=t_eid_matches] run function coinwars:base/gt_playerinit_loop_assign_eid_recursion

# This is for testing purposes to force fake conflicts and cause recursion
#execute if entity @a[tag=t_player_initialize,limit=1,scores={eid_self=50..}] run function coinwars:base/gt_playerinit_loop_assign_eid_recursion
