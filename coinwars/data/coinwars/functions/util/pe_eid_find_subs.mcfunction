# util/pe_eid_find_subs.mcfunction
#
# Context:
#	as: an entity
#
# Summary: Sets eid_compare for entities with eid_state == 1..2 accoridingly:
#	eid_compare == 0 iff their eid_owner == my eid_self
#	eid_compare > 0 otherwise

# Perform comparison
execute as @e[scores={eid_state=1..2}] run scoreboard players operation @s eid_compare = @s eid_owner
scoreboard players operation @e[scores={eid_state=1..2}] eid_compare -= @s eid_self
...
# Entities that declare them to be their own owner should not be considered their own subordinate
